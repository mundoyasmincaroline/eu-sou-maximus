  ...
  return (
    <div className="overflow-hidden">
      <section className="mx-auto max-w-3xl px-5 py-20 lg:px-10 lg:py-28">
        ...
        <div className="mt-12 overflow-hidden rounded-3xl border border-gold/20 bg-card/60 backdrop-blur shadow-luxe">
          ...
          <div className="p-7 md:p-10">
            <div className="mb-6 flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-muted-foreground">
              ...
              {goal && step > 0 && <span className="text-gold">{GOALS.find((g) => g.id === goal)?.title}</span>}
            </div>
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  {GOALS.map((g) => (
                    <button
                      ...
                      className={`group rounded-2xl border p-5 text-left transition-all ${goal === g.id ? "border-gold/70 bg-gold/10 shadow-glow-gold" : "border-gold/15 hover:border-gold/40"}`}
                    >
                      <g.icon className="h-6 w-6 text-gold" strokeWidth={1.5} />
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <Field label="Seu nome">
                  <input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-gold/20 bg-background/40 px-4 py-3 text-sm outline-none focus:border-gold/60" placeholder="Nome completo" />
                </Field>
                <Field label="Empresa, marca ou projeto (opcional)">
                  <input value={company} onChange={(e) => setCompany(e.target.value)} className="w-full rounded-xl border border-gold/20 bg-background/40 px-4 py-3 text-sm outline-none focus:border-gold/60" placeholder="Ex: Marca XYZ" />
                </Field>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-5">
                ...
                <Field label="O que você tem em mente?">
                  <textarea value={details} onChange={(e) => setDetails(e.target.value)} rows={5} className="w-full resize-none rounded-xl border border-gold/20 bg-background/40 px-4 py-3 text-sm outline-none focus:border-gold/60" placeholder="Descreva a proposta, formato, expectativas..." />
                </Field>
                {goal !== "mentoria" && (
                  <Field label="Data ou janela desejada">
                    <input value={date} onChange={(e) => setDate(e.target.value)} className="w-full rounded-xl border border-gold/20 bg-background/40 px-4 py-3 text-sm outline-none focus:border-gold/60" placeholder="Ex: Setembro/2026" />
                  </Field>
                )}
                <Field label="Investimento previsto (opcional)">
                  <div className="flex flex-wrap gap-2">
                    {["Até R$ 5 mil", "R$ 5–15 mil", "R$ 15–50 mil", "R$ 50 mil+"].map((b) => (
                      <button key={b} type="button" onClick={() => setBudget(b)} className={`rounded-full border px-4 py-2 text-xs ${budget === b ? "border-gold/70 bg-gold/15 text-gold" : "border-gold/20 text-muted-foreground hover:border-gold/40"}`}>{b}</button>
                    ))}
                  </div>
                </Field>
              </div>
            ...
            {step === 3 && (
              <div>
                <div className="grid h-14 w-14 place-items-center rounded-full gradient-gold text-[oklch(0.12_0.012_30)] shadow-glow-gold"><Check className="h-7 w-7" /></div>
                ...
                <pre className="mt-6 max-h-64 overflow-auto whitespace-pre-wrap rounded-2xl border border-gold/20 bg-background/60 p-5 text-sm leading-relaxed text-foreground/85">{message}</pre>
                <a href={waLink} target="_blank" rel="noreferrer" className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-full gradient-gold px-6 py-4 text-sm font-semibold text-[oklch(0.12_0.012_30)] shadow-glow-gold">
            ...
            {step < 3 && (
              <div className="mt-10 flex items-center justify-between">
                <button
                  ...
                  disabled={step === 0}
                  className="inline-flex items-center gap-2 rounded-full border border-gold/20 px-5 py-2.5 text-sm text-muted-foreground transition-colors hover:text-gold disabled:opacity-30"
                >
                ...
                <button
                  ...
                  disabled={!canNext()}
                  className="inline-flex items-center gap-2 rounded-full gradient-gold px-6 py-3 text-sm font-semibold text-[oklch(0.12_0.012_30)] shadow-glow-gold disabled:opacity-40 disabled:shadow-none"
                >
